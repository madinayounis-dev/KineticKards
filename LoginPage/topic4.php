...
// Function to insert user with all flashcard_ids from topic4_flashcards table
function insert_user_performance($con, $user_id) {
    $query = "INSERT INTO user_performance (user_id, flashcard_id, next_appearance) SELECT $user_id, flashcard_id, NOW() FROM flashcards.topic4_flashcards";
...
function get_flashcard_question_answer($con, $flashcard_id) {
    if ($flashcard_id !== null) {
        $query = "SELECT question, answer FROM flashcards.topic4_flashcards WHERE flashcard_id = $flashcard_id";
        $result = mysqli_query($con, $query);
