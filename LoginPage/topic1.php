...
// Function to insert user with all flashcard_ids from topic1_flashcards table
function insert_user_performance($con, $user_id) {
    $query = "INSERT INTO user_performance (user_id, flashcard_id, next_appearance) SELECT $user_id, flashcard_id, NOW() FROM flashcards.topic1_flashcards";
...
function get_flashcard_question_answer($con, $flashcard_id) {
    if ($flashcard_id !== null) {
        $query = "SELECT question, answer FROM flashcards.topic1_flashcards WHERE flashcard_id = ?";
...
$query = "SELECT mastery FROM user_performance WHERE user_id = ? AND flashcard_id = ?";
$stmt = mysqli_prepare($con, $query);
mysqli_stmt_bind_param($stmt, "ii", $user_id, $flashcard_id);
mysqli_stmt_execute($stmt);
$result = mysqli_stmt_get_result($stmt);

$mastery_level = 0;
if ($result && mysqli_num_rows($result) > 0) {
    $mastery_level = mysqli_fetch_assoc($result)['mastery'];
}
...
<div class="container">
        ...
        <div class="flashcard">
            ...
            <div class="flashcard-content" onclick="toggleCard(this)">
                <div class="flashcard-front">
                    <!-- PHP code to display question if available -->
                    <p><?php echo isset($flashcard_data['question']) ? $flashcard_data['question'] : ''; ?></p>
                </div>
                <div class="flashcard-back">
                    <!-- PHP code to display answer if available -->
    ...
    <div class="flashcard-navigation">
        <!-- Button for indicating a negative response -->
        ...
        <!-- Button for indicating a positive response -->
